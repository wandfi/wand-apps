...
export function BVaultRedeemAll({ bvc }: { bvc: BVaultConfig }) {
  ...
  return (
    <div className={cn('flex justify-between items-center gap-5 flex-wrap')}>
      ...
      <BBtn className='mx-auto mt-6 flex justify-center items-center gap-2' busy={isPending} disabled={disableRedeemAll} onClick={mutate as any}>Redeem All</BBtn>
    </div>
  )
}
      ...
      <div className={cn('flex flex-col md:flex-row gap-4 mt-2')}>
        ...
        <div className='flex flex-col gap-2 w-full'>
          <div className='flex gap-6 items-end font-semibold'>
            ...
            <span className='text-xs opacity-60'>Epoch {(current?.epochId || currentEpochId || 1n).toString()}</span>
          </div>
          <div className='flex-1 overflow-y-auto flex flex-col gap-4 font-semibold text-sm'>
            <div className='flex gap-10 items-end font-semibold'>
