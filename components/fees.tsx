...
export function Fees({ fees, def = '-' }: { fees: ({ name: string, value: number }[]) | string, def?: string }) {
    ...
    return <div className="flex gap-2 text-xs font-medium leading-none">Fees:
        {!isStr && fees.length > 0 ? <Tip className="underline underline-offset-2" contentClassName="whitespace-pre-wrap" node={`$${round(total, roundDecimal)}`}>
            {fees.map(item => `${item.name}: ${round(item.value, roundDecimal)}`).join("\n\r")}
